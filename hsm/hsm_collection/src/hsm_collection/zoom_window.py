...
class InteractiveWindow(KeyCommandedObject):
    ...
    def __init__(self, name, update_period = None):
        ...
        cv2.setMouseCallback(self.name,self.handleEvents,0)
        if update_period > 0:
    ...
    def update(self,char_str=None):
        ...
        self.show_image()
        if not char_str:
            keycode = cv2.waitKey(self.update_period)
...
class ZoomWindow(InteractiveWindow):
    ...
    def image_to_show( self ):
        """
        Return the image which you wish to be displayed in the window, before zooming
        """
        abstract
    ...
    def move(self, x, y):
        cv2.cv.MoveWindow(self.name, x, y )
...
class ImageViewer(ZoomWindow):
    
    def __init__(self, image, name = "Viewer", update_period = None, zoom_out = 1):
        self.image = image
        ZoomWindow.__init__(self,name,update_period,zoom_out)
